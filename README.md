...
- **`FilterOptions.disabledClassName` — distinct CSS class for `disabled` items** — new opt-in filter option. When set, nodes whose forbidden state comes from `disabled: true` get this class on `_forbiddenClassName` instead of the default `forbiddenClassName`. Lets consumers style "coming soon" placeholders distinctly from permission-denied items (e.g. amber "Unavailable" vs. red "Access restricted") with no renderer changes. Precedence rule: when both conditions apply, `disabledClassName` wins because the product-level signal is the more permanent one. Default `undefined` → falls back to `forbiddenClassName`, fully backward compatible.
